...
jdbcImportForeignSchema(ImportForeignSchemaStmt *stmt, Oid serverOid)
{
	...
	if (schema_list != NIL)
	{
		...
		foreach(table_lc, schema_list)
		{
			...
			if(!excluded) {
				char	   *table_name_quoted = jdbc_deparse_quote_identifier(tmpTableInfo->table_name, conn->q_char);
				resetStringInfo(&buf);
				if (recreate)
				{
				...
				foreach(column_lc, tmpTableInfo->column_info)
				{
					...
					appendStringInfo(&buf, "%s %s",
									 jdbc_deparse_quote_identifier(columnInfo->column_name, conn->q_char),
